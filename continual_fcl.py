...
class LightningModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        token_embeddings = x
        source = self.net_for_fingerprint(token_embeddings[:, 0, :])
...
class PropertyPredictionDataModule(pl.LightningDataModule):
    def __init__(self, hparams):
        ...
        self.train_sim = json.load(open(f'feature_extraction/{self.dataset_name}/similar_molecules_top5_train.json'))
        self.val_sim = json.load(open(f'feature_extraction/{self.dataset_name}/similar_molecules_top5_val.json'))
        self.test_sim = json.load(open(f'feature_extraction/{self.dataset_name}/similar_molecules_top5_test.json'))
    ...
    def collate(self, batch):
        ...
        return (torch.tensor(tokens['input_ids']), torch.tensor(tokens['attention_mask']), all_labels, self.train_ds.measure_mean, self.train_ds.measure_std, fcl_mask.long())
