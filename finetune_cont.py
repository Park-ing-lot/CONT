...
class LightningModule(pl.LightningModule):
    ...
    def validation_step(self, val_batch, batch_idx, dataset_idx):
        ...
        mask = val_batch[1]
        targets = val_batch[2]
        measure_mean = val_batch[3]
        measure_std = val_batch[4]
        ...
        loss, pred, actual = self.get_loss(loss_input, targets)
        ### 
        pred = (pred * measure_std) + measure_mean
        self.log('train_loss', loss, on_step=True)
        return {
            "val_loss": loss,
            "pred": pred.detach(),
...
def get_dataset(data_root, filename, dataset_len, aug, measure_name, normalize=False):
    df = pd.read_csv(os.path.join(data_root, filename))
    print("Length of dataset:", len(df))
    if dataset_len:
        df = df.head(dataset_len)
        print("Warning entire dataset not used:", len(df))
    dataset = PropertyPredictionDataset(df,  measure_name, aug, normalize=normalize)
    return dataset

class PropertyPredictionDataset(torch.utils.data.Dataset):
    def __init__(self, df,  measure_name, tokenizer, aug=True, normalize=False):
        ...
        if measure_name:
            if normalize:
                ...
                self.measure_mean = 0
                self.measure_std = 1
            # df[measure_name] = (df[measure_name]-df[measure_name].min())/(df[measure_name].max()-df[measure_name].min())
...
class PropertyPredictionDataModule(pl.LightningDataModule):
    ...
    def collate(self, batch):
        ...
        return (torch.tensor(tokens['input_ids']), torch.tensor(tokens['attention_mask']), torch.tensor([smile[1] for smile in batch]), self.train_ds.measure_mean, self.train_ds.measure_std)
